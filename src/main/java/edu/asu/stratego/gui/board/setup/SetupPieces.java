...
public class SetupPieces {
	private static Map<PieceType, MutableBoolean> pieceSelected =
            new EnumMap<>(PieceType.class);
	
	private static Map<PieceType, Integer> availability =
            new EnumMap<>(PieceType.class);
	
	private static Map<PieceType, ImageView> pieceImages =
            new EnumMap<>(PieceType.class);
	
	private static Map<PieceType, Label> pieceCount =
            new EnumMap<>(PieceType.class);
    ...
    public SetupPieces() {
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            pieceImages.put(pieceType, new ImageView(HashTables.
                    getPieceMap().get(playerColor + "_" + pieceSuffix[i])));
    ...
    private class SelectPiece implements EventHandler<MouseEvent> {
        ...
        private void setPieceType(PieceType p){
            if (availability.get(p) != 0)
                pieceImages.get(p).setEffect(new Glow(0.0));
            pieceSelected.get(p).setFalse();
        }
    }
