...
public class BoardSquareEventPane extends GridPane {
    ...
    private class OnHover implements EventHandler<MouseEvent> {
        ...
        public void handle(MouseEvent e) {
            ImageView hoverVar = (ImageView) e.getSource();
            int row = GridPane.getRowIndex(hoverVar);
            int col = GridPane.getColumnIndex(hoverVar);
    ...
    private class OffHover implements EventHandler<MouseEvent> {
        @Override
        public void handle(MouseEvent e) {
            ImageView hover1 = (ImageView) e.getSource();
            // Change the behavior of the off hover based on the game/move status
            if (Game.getStatus() == GameStatus.SETTING_UP)
                hover1.setImage(ImageConstants.HIGHLIGHT_NONE);
            else if (Game.getStatus() == GameStatus.WAITING_OPP)
                hover1.setImage(ImageConstants.HIGHLIGHT_NONE);
            else if (Game.getStatus() == GameStatus.IN_PROGRESS) {
                if (Game.getMoveStatus() == MoveStatus.OPP_TURN)
    ...
    private class SelectSquare implements EventHandler<MouseEvent> {
        ...
        private ArrayList<Point> computeValidMoves(int row, int col) {
            ...
            int max= isScout(pieceType);

            ArrayList<Point> validMoves1 = new ArrayList<>();
            ...
            availableMoves(pieceType,validMoves1,row,col, max);
            return validMoves1;
        }

        private void availableMoves (PieceType pieceType, List<Point> validMoves, int row, int col, int max){

            if (pieceType != PieceType.BOMB && pieceType != PieceType.FLAG) {
                // Negative Row (UP)
                rowUp(validMoves,row,col,max);
                // Positive Col (RIGHT)
                colRigth(validMoves,row,col,max);
                // Positive Row (DOWN)
                rowDown(validMoves,row,col,max);
                // Negative Col (LEFT)
                colLeft(validMoves,row,col,max);
            }
        }

        private void rowUp(List<Point> validMoves, int row, int col, int max){
            for (int i = -1; i >= -max; --i) {
                if (moving2(row, col, validMoves, i)) break;
            }
        }

        private void rowDown(List<Point> validMoves, int row, int col, int max){
            for (int i = 1; i <= max; ++i) {
                if (moving(row, col, validMoves, i)) break;
            }
        }

        private void colLeft(List<Point> validMoves, int row, int col, int max){
            for (int i = 1; i <= max; ++i) {
                if (moving2(row, col, validMoves, i)) break;
            }
        }

        private void colRigth(List<Point> validMoves, int row, int col, int max){
            for (int i = -1; i >= -max; --i) {
                if (moving(row, col, validMoves, i)) break;
            }
        }

        private int isScout(PieceType pieceType){
            if (pieceType==PieceType.SCOUT)
                return 8;
            else return 1;
        }
        private boolean moving2(int row, int col, List<Point> validMoves1, int i) {
            if (isInBounds(row + i, col) && (!isLake(row + i, col) || (!isNullPiece(row + i, col) && !isOpponentPiece(row + i, col)))) {
                if (isNullPiece(row + i, col) || isOpponentPiece(row + i, col)) {
                    validMoves1.add(new Point(row + i, col));

                    return !isNullPiece(row + i, col) && isOpponentPiece(row + i, col);
                } else
                    return true;
            } else
                return true;
        ...
        private boolean moving(int row, int col, List<Point> validMoves1, int i) {
            if (isInBounds(row, col + i) && (!isLake(row, col + i) || (!isNullPiece(row, col + i) && !isOpponentPiece(row, col + i)))) {
                if (isNullPiece(row, col + i) || isOpponentPiece(row, col + i)) {
                    validMoves1.add(new Point(row, col + i));

                    return !isNullPiece(row, col + i) && isOpponentPiece(row, col + i);
                } else
                    return true;
            } else
                return true;
        ...
        public void handle(MouseEvent e) {
            ...
            if (Game.getStatus() == GameStatus.SETTING_UP && isHoverValid(row, col)) {
                setUpAPiece(squarePiece,square,playerColor,squarePane);
            }else if (Game.getStatus() == GameStatus.IN_PROGRESS && Game.getTurn() == playerColor) {
                selectPieceToMove(row,col,playerColor);
            }
        }

        private void setUpAPiece(Piece squarePiece, ClientSquare square, PieceColor playerColor, BoardSquarePane squarePane){
            ...
            if (squarePiece != null) {
                ...
                // Replace the existing piece with the selected piece (in SetupPanel).
                    ...
                    squarePane.setPiece(HashTables.getPieceMap().get(square.getPiece().getPieceSpriteKey()));
                }
            }
        ...
        private void selectPieceToMove(int row, int col, PieceColor playerColor){
            ...
            if (Game.getMoveStatus() == MoveStatus.START_SELECTED && isValidMove(row, col)) {
                ...
                synchronized (ClientGameManager.getSendMove()) {
                    ClientGameManager.getSendMove().notifyAll();
                }
            }
            ...
            for (int row = 6; row < 10; ++row) {
                ...
                if (squarePiece == null) {
                    ...
                    while (pieceType == null) {
                        int randInt = r.nextInt(availTypes.size());
                        pieceType=getRandomPiece(pieceType,availTypes,randInt);
                    }
    ...
    private static PieceType getRandomPiece(PieceType pieceType, List<PieceType> availTypes, int randInt){
    ...
    public static void helpWindow() {
        ...
        f.add(label);
        ImageIcon instructions = new ImageIcon(ImageConstants.class.getResource("images/help/instrucciones.png"));
        Image image = instructions.getImage();
        ImageIcon iconScale = new ImageIcon(image.getScaledInstance(label.getWidth(),label.getHeight(), Image.SCALE_SMOOTH));
