...
public class SetupPanel {
    ...
    private static GridPane panelForSetUp = new GridPane();
    private static GridPane  piecePane         = new GridPane();
    private static final  Object    updateReadyStatus = new Object();
    ...
    private static ImageView buttomRandom      = new ImageView();
    private static final Logger LOGGER =Logger.getLogger(ServerGameManager.class.getName());

    //Boton ayuda
    private static ImageView helpButton = new ImageView();
     ...
     */
    private SetupPanel() {}
    public static void setupPanelIni(){
        final double UNIT = ClientStage.getUnit();

        panelForSetUp.setMaxHeight(UNIT * 6);
        panelForSetUp.setMaxWidth(UNIT * 10);

        // Panel background.
        String backgroundURL = "edu/asu/stratego/media/images/board/setup_panel.png";
        panelForSetUp.setStyle("-fx-background-image: url(" + backgroundURL + "); " +
        ...
        double fontScale = 1.0 / ((double)(titleContent.length() - 7) / 8 + 2);

        Label nameDisplay = new Label(titleContent);
        String letterType = "Century Gothic";
        nameDisplay.setFont(Font.font(letterType, FontWeight.BOLD, UNIT * fontScale));
        ...
        setupTimer.setFont(Font.font(letterType, UNIT*fontScale));
        setupTimer.setTextFill(new Color(0.9, 0.5, 0.0, 1.0));
        setupTimer.setAlignment(Pos.TOP_LEFT);

        buttomRandom.setImage(ImageConstants.RANDOM_BUTTON);
        buttomRandom.setFitHeight(UNIT*0.7);
        buttomRandom.setFitWidth(UNIT);
        buttomRandom.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> BoardSquareEventPane.randomSetup());

        //Help Button
        helpButton.setImage(ImageConstants.HELP_BUTTON);
        helpButton.setFitHeight(UNIT*0.7);
        helpButton.setFitWidth(UNIT);
        helpButton.addEventHandler(MouseEvent.MOUSE_CLICKED, e-> BoardSquareEventPane.helpWindow());
        ...
        headerText.add(buttomRandom,2,3);
        headerText.add(helpButton,2,0);
        ...
        panelForSetUp.add(headerPane, 0, 0);
        ...
        panelForSetUp.add(piecePane, 0, 1);
        ...
        readyButton.addEventHandler(MouseEvent.MOUSE_ENTERED_TARGET, e -> readyButton.setImage(ImageConstants.READY_HOVER));

        readyButton.addEventHandler(MouseEvent.MOUSE_EXITED_TARGET, e -> readyButton.setImage(ImageConstants.READY_IDLE));

        readyButton.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> Platform.runLater(SetupPanel::finishSetup));

        // Text properties.
        instructions.setFont(Font.font(letterType, UNIT * 0.3));
        ...
        panelForSetUp.add(instructionPane, 0, 2);
        ...
        readyLabel.setFont(Font.font(letterType, UNIT * 0.6));
     ...
     * @see UpdateReadyButton
     */
    static Object getUpdateReadyStatus() {
     ...
     */
    public static GridPane getPanelForSetUp() {
        return panelForSetUp;
    }
    ...
    public static void finishSetup() {
        ...
        synchronized (setupPieces) {
            panelForSetUp.getChildren().remove(instructionPane);
            panelForSetUp.getChildren().remove(piecePane);
            panelForSetUp.add(readyLabel, 0, 2);

            setupPieces.notifyAll();
        }
    }
            ...
            synchronized (updateReadyStatus) {
                while (true) {
                    try {
                        ...
                        LOGGER.log(Level.WARNING, "SetUp interrupted due to internal error", e);
